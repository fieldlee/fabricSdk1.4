...
var upgrade = require('./app/upgrate-chaincode.js');
...
// btcOrg_TOKEN=$(curl -s -X POST http://localhost:4000/login -H "content-type: application/x-www-form-urlencoded" -d 'username=btcOrg&password=password&orgName=btcOrg')
//  btcOrg_TOKEN=$(echo $btcOrg_TOKEN | jq ".token" | sed "s/\"//g")
...
//                   -H "authorization: Bearer $mmOrg_TOKEN" \
//                   -H "content-type: application/json" \
//                   -d '{"channelName":"mmchannel"}'
...
//                                 -H "authorization: Bearer $mmOrg_TOKEN" \
//                                 -H "content-type: application/json" \
//                                 -d '{"peers": ["peer0","peer1"],"channelName":"mmchannel"}'


app.post('/channels/peers', async function(req, res) {
...
app.put('/channels/chaincodes', async function(req, res) {
	...
	let message = await upgrade.upgrateChaincode(peers, channelName, chaincodeName, chaincodeVersion, chaincodeType, fcn, args, req.username, req.orgname);
	res.send(message);
});
